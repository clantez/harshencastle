...
public class CustomGuiStructure extends GuiScreen
{

	public CustomGuiStructure(TileEntityStructure structure) {
		this.tileStructure = structure;
    ...
    public void f(BlockPos pos, BlockPos size, Mirror mirror, Rotation rot, String metaDataEdie, boolean ignoreEntities,
    		boolean showAir, boolean showBoundingBox, float integrirty, long seed)
    {
    	sendCommandToServer(1, pos, size, mirror, rot, metaDataEdie, showBoundingBox, showBoundingBox, showBoundingBox, integrirty, seed);
    }
    
    private void sendCommandToServer(int p_189820_1_,BlockPos pos, BlockPos size, Mirror mirror, Rotation rot, String metaDataEdit, boolean ignoreEntities,
    		boolean showAir, boolean showBoundingBox, float integrirty, long seed)
    {
        try
        {
            ...
            packetbuffer.writeInt(size.getX());
            packetbuffer.writeInt(size.getY());
            packetbuffer.writeInt(size.getZ());
            packetbuffer.writeString(mirror.toString());
            packetbuffer.writeString(rot.toString());
            packetbuffer.writeString(metaDataEdit);
            packetbuffer.writeBoolean(ignoreEntities);
            packetbuffer.writeBoolean(showAir);
            packetbuffer.writeBoolean(showBoundingBox);
            packetbuffer.writeFloat(integrirty);
            packetbuffer.writeVarLong(seed);
            Minecraft.getMinecraft().getConnection().sendPacket(new CPacketCustomPayload("MC|Struct", packetbuffer));
        }
