...
public class BaseLargeTreeGenerator extends WorldGenAbstractTree
{
    ...
    List<BaseLargeTreeGenerator.FoliageCoordinates> foliageCoords;
    
    private final IBlockState leaves;
    private final BlockLog log;

    public BaseLargeTreeGenerator(boolean notify, IBlockState leaves, BlockLog log)
    {
        super(notify);
        this.leaves = leaves;
        this.log = log;
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<BaseLargeTreeGenerator.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new BaseLargeTreeGenerator.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new BaseLargeTreeGenerator.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeaves()
    {
        for (BaseLargeTreeGenerator.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    void generateLeafNodeBases()
    {
        for (BaseLargeTreeGenerator.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
            ...
            {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates, log);
            }
        }
    }
    ...
    private boolean validTreeLocation()
    {
        ...
        if(!Arrays.asList(HarshenBlocks.harshen_dimensional_dirt, HarshenBlocks.harshen_dimensional_rock).contains(state.getBlock()))
        	return false;
